...
def make_wt_single(obs_id_dir,tres='0.0001220703125',tseg='128.0',
    ...
    # Logging
    if log_name is None:
        log_name = make_logger('make_nicer_wt_single',outdir=obs_id_dir)
    ...
    for i,lc in enumerate(main_lc_list):
        ...
        if min_scale is None: min_scale = lc_rebin.tres*4
        if max_scale is None: max_scale = np.round(lc_rebin.texp/2,1)
        ...
        text2 = 't_res: {}, t_exp: {}, min_s: {}, max_s: {}'.\
            format(lc_rebin.tres, np.round(lc_rebin.texp,1), min_scale, max_scale)
        
        # Computing Mexican hat wavelet
        wltmh_file_name = 'wlt_{}_S{}_{}_mexhat_{}.pkl'.\
            format(main_prod_name,min_scale,max_scale,i+1)
        if not (wt_prod_dir/wltmh_file_name).is_file():
            wltmh = WaveletTransform.\
                from_lc(lc_rebin,s_min=min_scale,s_max=max_scale,dj=dj,family='mexhat')
            wltmh.save(file_name=wltmh_file_name,fold=wt_prod_dir)
        else:
            wltmh = WaveletTransform.load(file_name=wltmh_file_name,fold=wt_prod_dir)
            
        # Computing Morlet wavelet
        wltmo_file_name = 'wlt_{}_S{}_{}_morlet_{}.pkl'.\
            format(main_prod_name,min_scale,max_scale,i+1)
        if not (wt_prod_dir/wltmo_file_name).is_file():
            wltmo = WaveletTransform.from_lc(lc_rebin,s_min=min_scale,s_max=max_scale,dj=dj,family='morlet')
            wltmo.save(file_name=wltmo_file_name,fold=wt_prod_dir)
        else:
            wltmo = WaveletTransform.load(file_name=wltmh_file_name,fold=wt_prod_dir) 
            
        # Plotting wavelet transforms
        wlt_plot_name = 'wlt_{}_S{}_{}_{}.jpeg'.format(main_prod_name,min_scale,max_scale,i+1)
        if not (wt_plot_dir/wlt_plot_name).is_file():
            ...
            fig.savefig(str(wt_plot_dir/wlt_plot_name), dpi=300, bbox_inches = 'tight',
                pad_inches = 0)
            # ---------------------------------------------------------
        ...
        pdf.image(str(wt_plot_dir/wlt_plot_name),x=coors[0],y=coors[1],w=coors[2]-coors[0])
    # -----------------------------------------------------------------

    # Saving the PDF file
    pdf_name = wt_plot_dir/'{}_wlt_{}_S_{}_{}.pdf'.format(obs_id,main_prod_name,min_scale,max_scale)

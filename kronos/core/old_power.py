...
class PowerSpectrum(pd.DataFrame):
    ...
    def from_lc(lightcurve):
        ...
        if 'List' in str(lightcurve.__class__) or type(lightcurve) == list:
            ...
            meta_data['CREATION_MODE'] = 'Power computed from LightcurveList'
            meta_data['T_RES'] = lightcurve[0].tres
            ...
            meta_data['CREATION_MODE'] = 'Power computed from Lightcurve'
            meta_data['T_RES'] = lightcurve.tres
            ...
            if lightcurve.counts.sum() > 0:
                ...
                power = PowerSpectrum(freq,np.multiply(amp, np.conj(amp)).real,
                                ...
                                weight_value=1,meta_data=meta_data)
            else:
                power = PowerSpectrum()
...
class PowerList(list):
    ...
    def average_leahy(self, exc=[]):
        ...
        if len(self) != 0:
            ...
            changes['SEG_DUR'] = self[0].meta_data['SEG_DUR']
            changes['T_RES'] = self[0].meta_data['T_RES']
